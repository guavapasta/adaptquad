...
function equipartBezAnimate(Ptsx, Ptsy, C, n)
    ...
    %example on calling this function equipartBezAnimate(0, 0, @(r) r^(1), 10)
    ...
    listT1 = [];
    iter = 1;
    for s = 0:0.01:1    %can use either bisection or newton iteration, preference.
            listT1(iter) = distanceAlongParametric(Px, Py, 0, 1, C(s)); %distanceAlongParametricNewton(Px, Py, s, s); % distanceAlongParametric(Px, Py, 0, 1, C(s));
            iter = iter + 1;
    end
    ...
    for i = 1:size(Ptsx, 1)
        ...
        for s = 0.01:0.01:1
            ...
            clearpoints(pointer); clearpoints(pointer2); addpoints(pointer,x,y);   addpoints(pointer2,x2,y2);
            drawnow;pause(0.01)
        end
    end
end